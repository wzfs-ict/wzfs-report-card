...
export default function App() {
  ...
  // Attendance file: plain single-sheet format; one row per student
  const readAttendanceFile = useCallback((file, onData) => {
    const reader = new FileReader();
    reader.onload = (e) => {
      try {
        const wb = XLSX.read(e.target.result, { type: "array" });
        ...
        onData(rows);
        setErrors([]);
      } catch { setErrors(["Could not read attendance file — please check the format."]); }
    };
    reader.readAsArrayBuffer(file);
  }, []);
  ...
  const handleGenerate = () => {
    ...
    try {
      ...
      let merged = mergeStudentData(parsed, awards).map(s => ({
        ...s,
      ...
      // Merge attendance data when an attendance file was uploaded
      if (attendanceRaw) {
        const attMap = parseAttendanceData(attendanceRaw);
        merged = mergeAttendanceData(merged, attMap);
      }
  ...
  if (step === "preview") {
    return (
      <div className="app-preview">
        ...
        {showSigManager && <SignatureManager onClose={refreshSignatures} principalName={meta.principal}/>}
      </div>
    );
  }
    ...
    <div className="app-upload">
      ...
      <main className="upload-main">
        ...
        <div className="upload-grid">
          ...
          <UploadPanel title="Attendance File" description="Student_ID + Name + School Days, Present, Absences, Tardy" icon="📅"
            onUpload={f=>readAttendanceFile(f,setAttendanceRaw)} uploaded={!!attendanceRaw} rowCount={attendanceRaw?.length} required={false}/>
        </div>
